terraform {
  ...
  backend "s3" {
    ...
    key     = "terraform/v1/production/ecs/monitoring/services/grafana"
    encrypt = true
...
data "terraform_remote_state" "production_ecs_monitoring" {
  backend = "s3"

  config = {
    bucket  = "pickstudio-infrastructure"
    key     = "terraform/v1/production/ecs/monitoring"
    region  = "ap-northeast-2"
    encrypt = true
  }
}
...
data "terraform_remote_state" "production_lb_buddystock" {
  ...
  config = {
    bucket  = "pickstudio-infrastructure"
    key     = "terraform/v1/production/lb/monitoring"
    region  = "ap-northeast-2"
    encrypt = true
  }
}
